...
# Align all dates to start of the week
...
# Load existing GRU results and forecasts, or initialize empty containers
if (file.exists("results_combined_gru.csv")) {
  results_combined <- read.csv("results_combined_gru.csv")
  processed_coins <- results_combined$Coin
  combined_forecasts <- readRDS("combined_forecasts_gru.rds")
} else {
...
for (coin in tickers) {
  ...
  for (n_lag in 1:max_lag) {
    ...
    # Merge all relevant features into one dataset
    df <- crypto_returns %>%
    ...
    # Skip if not enough data for the current lag
    ...
    # Format input sequences for GRU model
    ...
    # Keep track of the best lag based on MAE
    if (mae < best_mae) {
      best_mae <- mae
      best_rmse <- rmse
      best_lag <- n_lag
  ...
  # Save best result for this coin
  if (!is.na(best_lag)) {
    ...
    write.csv(results_combined, "results_combined_gru.csv", row.names = FALSE)
    saveRDS(combined_forecasts, "combined_forecasts_gru.rds")
...
cat("Overall MAE:", round(mean(results_combined$MAE), 4), "\n")
cat("Overall RMSE:", round(mean(results_combined$RMSE), 4), "\n")

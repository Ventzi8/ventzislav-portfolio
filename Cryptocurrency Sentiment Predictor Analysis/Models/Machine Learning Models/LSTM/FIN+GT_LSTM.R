...
for (coin in tickers) {
  ...
  for (n_lag in 1:max_lag) {
    ...
    # Format data for LSTM model input
    ...
    train_lstm <- create_lstm_data(train_scaled, y_train, n_lag)
    val_lstm   <- create_lstm_data(val_scaled, y_val, n_lag)
    test_lstm  <- create_lstm_data(test_scaled, y_test, n_lag)
